...
public class AdminService extends ClientService {
	...
	public Company addCompany(Company company) throws ServiceException {
		if (companyRepositories.existsById(company.getId()))
			throw new ServiceException("AdminService Error: adding company failed, company already exists");
		else if (!companyRepositories.findByName(company.getName()).isEmpty())
			throw new ServiceException("AdminService Error: adding company failed, company name already exists");
		else if (!companyRepositories.findByEmail(company.getEmail()).isEmpty())
			throw new ServiceException("AdminService Error: adding company failed, company email already exists");
		else
	...
	public Company updateCompany(Company company) throws ServiceException {
		...
		if (optional.isEmpty())
			throw new ServiceException("AdminService Error: updating company failed, did not find required company id");
		else if (!optional.get().getName().equalsIgnoreCase(company.getName()))
			throw new ServiceException("AdminService Error: updating company failed, can not update company's name");
		else if (!companyRepositories.findByIdIsNotAndEmail(company.getId(), company.getEmail()).isEmpty())
			throw new ServiceException("AdminService Error: updating company failed, company email already exists");
		else
			return companyRepositories.save(company);
	}
	...
	public void deleteCompany(int companyID) throws ServiceException {
		if (companyRepositories.existsById(companyID)) {
			companyRepositories.deleteById(companyID);
		} else {
			throw new ServiceException("AdminService Error: deleting company failed, did not find required company id");
		}
	}
	...
	public Company getCompany(int companyID) throws ServiceException {
		...
		if (optional.isPresent()) {
			...
			throw new ServiceException("AdminService Error: getting company failed, did not find required company");
		}
	}
	...
	public void addCustomer(Customer customer) throws ServiceException {
		if (!customerRepositories.findByEmail(customer.getEmail()).isEmpty()) {
            throw new ServiceException("AdminService Error: adding customer failed, customer email already exists");
        } else {
	...
	public void updateCustomer(Customer customer) throws ServiceException {
		...
		if (optional.isEmpty()) {
			throw new ServiceException("AdminService Error: updating customer failed, did not find required customer id");
		} else if (!customerRepositories.findByIdIsNotAndEmail(customer.getId(), customer.getEmail()).isEmpty()) {
			throw new ServiceException("AdminService Error: updating customer failed, customer email already exists");
		} else {
			customerRepositories.save(customer);
		}
	}
